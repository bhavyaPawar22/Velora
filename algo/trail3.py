...
class DestroyOperators:
    ...
    def shaw_removal(self, sol: Solution, q: int) -> Tuple[Solution, List[str]]:
        ...
        for eid in assigned:
            ...
            dist, _ = self.state.constraints.distance(seed_emp.pickup, emp.pickup)
...
class RepairOperators:
    ...
    def _assign_with_noise(self, sol: Solution, eid: str) -> bool:
        ...
        # Add noise to each option's objective
        # Noise magnitude is ~20% of the average objective
        avg_obj = sum(o[0] for o in options) / len(options) if options else 1.0
        noise_scale = max(0.1, avg_obj * 0.2)
        
        noisy_options = []
        for obj, sched_idx, trip_idx, is_new, sequence in options:
            noisy_obj = obj + random.uniform(-noise_scale, noise_scale)
...
class ALNS:
    def __init__(self, state: ProblemState, config: ALNSConfig = None):
        ...
        self.destroy_ops = [
            self.destroy.random_removal,       # 1. Random
            self.destroy.worst_removal,         # 2. Worst (cost-based, fixed)
            self.destroy.trip_removal,          # 3. Trip removal
            self.destroy.shaw_removal,          # 4. Shaw (relatedness)
            self.destroy.time_window_removal,   # 5. Time-window inefficiency
            self.destroy.vehicle_cluster_removal # 6. Vehicle-type pressure
        ]
        
        self.repair_ops = [
            self.repair.greedy_insertion,                        # 1. Greedy
            lambda s, r: self.repair.regret_insertion(s, r, 2),  # 2. Regret-2
            lambda s, r: self.repair.regret_insertion(s, r, 3),  # 3. Regret-3
            self.repair.noise_insertion,                          # 4. Noise
            self.repair.priority_aware_insertion                  # 5. Priority-aware
        ]
        
        # Adaptive weights for operator selection
        ...
        self.decay = 0.8  # Weight decay factor
    ...
    def _update_weights(self, segment_size: int = 100):
        """Update adaptive weights based on operator performance."""
        for i in range(len(self.destroy_weights)):
            if self.destroy_counts[i] > 0:
                ...
                self.destroy_weights[i] = (
                    self.decay * self.destroy_weights[i] +
        ...
        for i in range(len(self.repair_weights)):
            if self.repair_counts[i] > 0:
                ...
                self.repair_weights[i] = (
                    self.decay * self.repair_weights[i] +
                    (1 - self.decay) * avg_score
                )
    ...
    def _run_alns_loop(self, current_sol: Solution, best_sol: Solution, best_cost: float):
        ...
        # Reset adaptive weights for this run
        self.destroy_weights = [1.0] * len(self.destroy_ops)
        self.repair_weights = [1.0] * len(self.repair_ops)
        self.destroy_scores = [0.0] * len(self.destroy_ops)
        self.repair_scores = [0.0] * len(self.repair_ops)
        ...
        while iteration < self.cfg.max_iter and no_improve < self.cfg.max_no_improve:
            ...
            if iteration % segment_size == 0:
                self._update_weights(segment_size)
