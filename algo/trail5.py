    ...
    def load(filepath: str) -> Tuple[List[Any], List[Any], Any, Dict]:
        ...
        try:
            ...
            for _, row in meta_df.iterrows():
                key = str(row.get('key', row.get('parameter', ''))
                          ).lower().strip()
            ...
            print(
                f"Successfully loaded metadata: α={metadata['alpha']}, β={metadata['beta']}")
        ...
        for _, r in base_df.iterrows():
            ...
            b_time = r.get('baseline_time', r.get(
                'time', r.get('baseline_travel_time', 
                r.get('baseline_time_min', 0))))
        ...
        office = Location(emp_df['drop_lat'].iloc[0],
                          emp_df['drop_lng'].iloc[0])
        ...
        for _, r in emp_df.iterrows():
            ...
            # Get baseline info
            b_info = baseline_data.get(
                r['employee_id'], {'cost': 0, 'time': 0})
            ...
            emp = Employee(
                ...
                vehicle_preference=VehiclePreference(
                    vp) if vp in ['premium', 'normal', 'any'] else VehiclePreference.ANY,
                sharing_preference=SharingPreference(
                    sp) if sp in ['single', 'double', 'triple'] else SharingPreference.TRIPLE,
            ...
            emp.baseline_value = (
                metadata['alpha'] * b_info['cost']) + (metadata['beta'] * b_info['time'])
...
class TripConstraints:
    ...
    def check_time_constraint(self, vehicle: Vehicle, employee_ids: List[str],
        ...
        deadline = min(
            self.employees[eid].adjusted_latest_drop for eid in employee_ids)
        ...
        for eid in pickup_sequence:
            ...
            travel = self.travel_time(
                current_loc, emp.pickup, vehicle.avg_speed)
            total_dist += dist
            final_route += route[:-1]
    ...
    def check_capacity_sharing(self, vehicle: Vehicle, employee_ids: List[str]) -> Tuple[bool, Dict]:
        ...
        sharing_limits = [
            self.employees[eid].max_passengers for eid in employee_ids]
    ...
    def is_trip_feasible(self, vehicle: Vehicle, employee_ids: List[str],
        ...
        cap_ok, cap_details = self.check_capacity_sharing(
            vehicle, employee_ids)
    ...
    def find_best_sequence(self, vehicle: Vehicle, employee_ids: List[str],
        ...
        if len(employee_ids) == 1:
            ok, _ = self.check_time_constraint(
                vehicle, employee_ids, employee_ids, start_time, start_location)
        ...
        if len(employee_ids) <= 6:
            ...
            for perm in permutations(employee_ids):
                seq = list(perm)
                ok, details = self.check_time_constraint(
                    vehicle, employee_ids, seq, start_time, start_location)
            ...
            sorted_emps = sorted(
                employee_ids, key=lambda eid: self.employees[eid].earliest_pickup)
            ok, _ = self.check_time_constraint(
                vehicle, employee_ids, sorted_emps, start_time, start_location)
...
class ProblemState:
    def __init__(self, employees: List[Employee], vehicles: List[Vehicle],
        ...
        self.office = office
        self.constraints = TripConstraints(
            self.employees, self.vehicles, office)
    ...
    def solution_cost(self, sol: Solution) -> Tuple[float, Dict]:
        ...
        objective = self.alpha * total_cost / self.sum_baseline_cost + \
            self.beta * total_time / self.sum_baseline_time
...
class InitialSolutionBuilder:
    ...
    def _phase1_savings_calculation(self):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                entry = self._compute_pair_savings(
                    emp_i, emp_j, common_vehicles)
    ...
    def _compute_individual_metrics(self):
        ...
        for emp in self.state.emp_list:
            ...
            for vehicle in self.state.veh_list:
                ...
                cap_ok, _ = self.constraints.check_capacity_sharing(vehicle, [
                                                                    emp.id])
                if not cap_ok:
                    continue
    ...
    def _compute_pair_savings(self, emp_i: str, emp_j: str,
        ...
        for vid in common_vehicles:
            ...
            cap_ok, _ = self.constraints.check_capacity_sharing(
                vehicle, [emp_i, emp_j])
            if not cap_ok:
                continue

            # CONSTRAINT 3: Check vehicle type for pair
            type_ok, _ = self.constraints.check_vehicle_type(
                vehicle, [emp_i, emp_j])
            if not type_ok:
                continue
            ...
            combined_time = details['arrival_at_office'] - \
                vehicle.available_from

            # Get individual metrics
            cost_i = self._individual_costs.get(
                emp_i, {}).get(vid, float('inf'))
            cost_j = self._individual_costs.get(
                emp_j, {}).get(vid, float('inf'))
            time_i = self._individual_times.get(
                emp_i, {}).get(vid, float('inf'))
            time_j = self._individual_times.get(
                emp_j, {}).get(vid, float('inf'))
    ...
    def _step2_regret_insertion(self, solution: 'Solution', assigned: Set[str]) -> Set[str]:
        ...
        """
        unassigned = [
            e.id for e in self.state.emp_list if e.id not in assigned]

        while unassigned:
            best_emp = None
            best_regret = float('-inf')
            # (objective, schedule_idx, trip_idx, is_new_trip)
            best_insertion = None
            ...
            for eid in unassigned:
                ...
                c2 = insertion_options[1][0] if len(
                    insertion_options) >= 2 else float('inf')
        ...
        for sched_idx, schedule in enumerate(solution.schedules):
            ...
            for trip_idx, trip in enumerate(schedule.trips):
                ...
                if time_ok:
                    ...
                    options.append(
                        (objective, sched_idx, trip_idx, False, best_seq))
    ...
    def _update_subsequent_trips(self, schedule: 'VehicleSchedule', start_idx: int):
        ...
        for i in range(start_idx, len(schedule.trips)):
            if i == 0:
                ...
                prev_end = schedule.trips[i -
                                          1].arrival_at_office + self.constraints.DROP_TIME
...
class DestroyOperators:
    ...
    def worst_removal(self, sol: Solution, q: int) -> Tuple[Solution, List[str]]:
        ...
        Marginal cost = solution_cost_with_employee - solution_cost_without_employee.
        ...
        for _ in range(q):
            ...
            # Calculate marginal cost for each assigned employee
            base_cost, _ = self.state.solution_cost(s)
            marginal_costs = []

            for eid in assigned:
                # Create a temporary copy with this employee removed
                temp = s.copy()
                self._remove(temp, eid)
                reduced_cost, _ = self.state.solution_cost(temp)
                # Marginal cost = how much removing this employee saves
                marginal = base_cost - reduced_cost
                marginal_costs.append((eid, marginal))

            # Sort by marginal cost descending (highest cost contribution first)
            marginal_costs.sort(key=lambda x: x[1], reverse=True)

            # Select using determinism parameter p:
            # Pick index = floor(random^p * len) which biases toward index 0 (worst)
            idx = int(random.random() ** self.p * len(marginal_costs))
            idx = min(idx, len(marginal_costs) - 1)

            chosen_eid = marginal_costs[idx][0]
    ...
    def shaw_removal(self, sol: Solution, q: int) -> Tuple[Solution, List[str]]:
        ...
        for eid in assigned:
            ...
            # Spatial relatedness: distance between pickups
            dist, _ = self.state.constraints.distance(
                seed_emp.pickup, emp.pickup)
            ...
            score = (dist / 50.0) + (time_diff / 120.0) + \
                vtype_diff + share_diff
            ...
            for trip in schedule.trips:
                if len(trip.employees) <= 1:
                    ...
                    if trip.employees:
                        ...
                        inefficiency_scores.append(
                            (eid, max(0.0, inefficiency)))
                    continue
                ...
                for eid in trip.employees:
                    ...
                    seq_idx = trip.pickup_sequence.index(
                        eid) if eid in trip.pickup_sequence else 0
                    ...
                    if seq_idx == 0:
                        ...
                        prev_time = trip.pickup_times.get(
                            prev_eid, trip.start_time)
                    ...
                    wait_time = max(
                        0.0, emp.earliest_pickup - arrival_at_pickup)
                    ...
                    dist_with, _ = self.state.constraints.distance(
                        prev_loc, emp.pickup)
                    dist_with2, _ = self.state.constraints.distance(
                        emp.pickup, next_loc)
                    dist_without, _ = self.state.constraints.distance(
                        prev_loc, next_loc)
...
class RepairOperators:
    ...
    def _compute_dmax(self) -> float:
        """Compute maximum distance between any two employee pickup locations."""
        emps = list(self.state.employees.values())
        dmax = 0.0
        # For efficiency, sample if too many employees
        if len(emps) <= 50:
            candidates = emps
        else:
            candidates = random.sample(emps, 50)
        for i in range(len(candidates)):
            for j in range(i + 1, len(candidates)):
                dist, _ = self.state.constraints.distance(
                    candidates[i].pickup, candidates[j].pickup
                )
                if dist > dmax:
                    dmax = dist
    ...
    def regret_insertion(self, sol: Solution, removed: List[str], k: int = 2) -> Solution:
        ...
        while unassigned:
            best_emp = None
            best_regret = float('-inf')
            # (sched_idx, trip_idx, is_new, sequence)
            ...
            if is_new:
                ...
                self.builder._insert_into_trip(
                    s, best_emp, sched_idx, trip_idx, sequence)
    ...
    def _assign_with_noise(self, sol: Solution, eid: str) -> bool:
        ...
        for obj, sched_idx, trip_idx, is_new, sequence in options:
            ...
            noisy_obj = obj + noise
            noisy_options.append(
                (noisy_obj, sched_idx, trip_idx, is_new, sequence))
    ...
    def priority_aware_insertion(self, sol: Solution, removed: List[str]) -> Solution:
        ...
        for eid in removed:
            ...
            # Count feasible insertion positions (fewer = more constrained)
            options = self._get_all_insertion_options(s, eid)
            if len(options) == 0:
                score += 20.0  # Extremely constrained
            elif len(options) <= 2:
                score += 8.0
            elif len(options) <= 5:
                score += 3.0
    ...
    def _get_all_insertion_options(self, sol: Solution, eid: str) -> List[Tuple]:
        ...
        for sched_idx, schedule in enumerate(sol.schedules):
            ...
            for trip_idx, trip in enumerate(schedule.trips):
                ...
                # CONSTRAINT 2: Check capacity & sharing
                cap_ok, _ = constraints.check_capacity_sharing(
                    vehicle, test_emps)
                if not cap_ok:
                    continue
                ...
                if time_ok and 'arrival_at_office' in details and 'total_distance' in details:
                    objective = self.builder._compute_objective(
                        details, vehicle)
                    options.append(
                        (objective, sched_idx, trip_idx, False, best_seq))
...
class ALNSConfig:
    def __init__(self):
        ...
        self.temp_accept_prob = 0.5
        self.num_runs = 20
        # Local search perturbation interval (paper: ψ = 200)
        self.perturbation_interval = 200
...
class ALNS:
    def __init__(self, state: ProblemState, config: ALNSConfig = None):
        ...
        self.pair_weights = [
            [1.0] * self.n_repair for _ in range(self.n_destroy)]
        self.pair_scores = [
            [0.0] * self.n_repair for _ in range(self.n_destroy)]
    ...
    def solve(self, verbose=True) -> Tuple[Solution, Dict]:
        ...
        if verbose:
            ...
            print(
                f"  Destroy ops: {self.n_destroy}, Repair ops: {self.n_repair}, Pairs: {self.n_destroy * self.n_repair}")
            print(f"  Perturbation interval: {self.cfg.perturbation_interval}")
            print(
                f"{'Run':<5} | {'Init Cost':<10} | {'Final Cost':<10} | {'Assigned':<10} | {'Improv %':<10} | {'Status'}")
            print("-" * 65)
        ...
        for run_idx in range(self.cfg.num_runs):
            ...
            if is_new_global_best:
                ...
                _, self.global_best_breakdown = self.state.solution_cost(
                    run_best_sol)
                status = "🏆 NEW BEST"

            if verbose:
                improv_pct = ((initial_cost - run_best_cost) /
                              initial_cost * 100) if initial_cost > 0 else 0.0
                print(
                    f"{run_idx+1:<5} | {initial_cost:<10.5f} | {run_best_cost:<10.5f} | {run_assigned:<10} | {improv_pct:<9.1f}% | {status}")
        ...
        if verbose:
            ...
            if self.global_best_sol:
                ...
                print(
                    f"Final Best Cost: {self.global_best_cost:.2f} (Assigned: {final_assigned}/{self.state.total_employees})")
    ...
    def _local_search_perturbation(self, sol: Solution) -> Tuple[Solution, float]:
        ...
        2. Inter-trip: Try swapping employees between trips on the same vehicle
        ...
        while improved and ls_iter < max_ls_iterations:
            improved = False
            ls_iter += 1
            ...
            for schedule in sol.schedules:
                ...
                for trip in schedule.trips:
                    ...
                    # Try all pairwise swaps in pickup sequence
                    ...
                    for i in range(n_emps):
                        for j in range(i + 1, n_emps):
                            # Create new sequence with i and j swapped
                            new_seq = current_seq[:]
                    ...
                    # Also try 2-opt reversal segments for sequences > 3
                    if n_emps >= 3:
                        for i in range(n_emps - 1):
                            for j in range(i + 2, n_emps + 1):
                                new_seq = current_seq[:i] + \
                                    current_seq[i:j][::-1] + current_seq[j:]
                    ...
                    if best_details is not None and best_seq != current_seq:
                        ...
                        trip.pickup_times = best_details.get(
                            'pickup_times', {})
                        trip.distance_km = best_details.get(
                            'total_distance', 0)
                        trip.route = best_details.get('route', [])
                        improved = True

            # --- Phase 2: Inter-trip employee relocation ---
            # Try moving one employee from one trip to another on the same vehicle
            for schedule in sol.schedules:
                vehicle = schedule.vehicle
                if len(schedule.trips) < 2:
                    continue
                ...
                for src_idx in range(len(schedule.trips)):
                    ...
                    for eid in src_trip.employees[:]:
                        ...
                        for dst_idx in range(len(schedule.trips)):
                            ...
                            cap_ok, _ = self.state.constraints.check_capacity_sharing(
                                vehicle, new_dst_emps
                            )
                            if not cap_ok:
                                continue

                            type_ok, _ = self.state.constraints.check_vehicle_type(
                                vehicle, new_dst_emps
                            )
                            if not type_ok:
                                continue
                            ...
                            dst_seq = self.state.constraints.find_best_sequence(
                                vehicle, new_dst_emps,
                                dst_trip.start_time, dst_trip.start_location
                            )
                            if not dst_seq:
                                continue

                            time_ok, dst_details = self.state.constraints.check_time_constraint(
                                vehicle, new_dst_emps, dst_seq,
                                dst_trip.start_time, dst_trip.start_location
                            )
                            if not time_ok:
                                continue
                            ...
                            new_src_emps = [
                                e for e in src_trip.employees if e != eid]
                            new_src_seq = [
                                e for e in src_trip.pickup_sequence if e != eid]

                            src_ok, src_details = self.state.constraints.check_time_constraint(
                                vehicle, new_src_emps, new_src_seq,
                                src_trip.start_time, src_trip.start_location
                            )
                            ...
                            new_dist = (src_details.get('total_distance', 0) +
                                        dst_details.get('total_distance', 0))
                            ...
                            if new_dist < old_dist:
                                ...
                                src_trip.pickup_times = src_details.get(
                                    'pickup_times', {})
                                src_trip.distance_km = src_details.get(
                                    'total_distance', 0)
                                ...
                                dst_trip.pickup_times = dst_details.get(
                                    'pickup_times', {})
                                dst_trip.distance_km = dst_details.get(
                                    'total_distance', 0)
                                dst_trip.route = dst_details.get('route', [])

                                improved = True
                                break  # Re-evaluate after making a move
    ...
    def _run_alns_loop(self, current_sol: Solution, best_sol: Solution, best_cost: float):
        ...
        temp = -(self.cfg.temp_start / math.log(self.cfg.temp_accept_prob)
                 ) * curr_cost if curr_cost > 0 else 100
        ...
        while iteration < self.cfg.max_iter and no_improve < self.cfg.max_no_improve:
            ...
            # --- Change #4: Select paired (d,r) operator ---
            ...
            # Determine removal size q (Change #1: dynamic range)
            ...
            if accept:
                if (new_assigned > best_assigned or
                    ...
                    score = self.sigma1  # New best
                elif new_cost < curr_cost:
                    score = self.sigma2  # Better than current
                else:
                    score = self.sigma3  # Accepted by SA
            ...
            # --- Change #4: Update paired weights every segment (Paper Eq. 15) ---
            if iteration % segment_size == 0:
                self._update_pair_weights()

            # --- Change #2: Local search perturbation every ψ iterations ---
            if iteration % psi == 0:
                current, curr_cost = self._local_search_perturbation(current)

                # Check if perturbation found new best
                perturb_assigned = len(current.all_assigned())
                if (perturb_assigned > best_assigned or
                        (perturb_assigned == best_assigned and curr_cost < best_cost)):
                    best_sol = current.copy()
                    best_cost = curr_cost
                    best_assigned = perturb_assigned
                    no_improve = 0
...
class ResultsVerifier:
    ...
    def verify_and_display(self, solution) -> Dict:
        ...
        baseline_cost_total = sum(
            self.state.employees[eid].baseline_cost for eid in assigned_ids)
        baseline_time_total = sum(
            self.state.employees[eid].baseline_time for eid in assigned_ids)
        ...
        baseline_weighted_total = sum(
            getattr(self.state.employees[eid], 'baseline_value', 0) for eid in assigned_ids)
        ...
        results['Office'] = (float(self.state.office.lat),
                             float(self.state.office.lng))
        ...
        for schedule in solution.schedules:
            ...
            for i, trip in enumerate(schedule.trips):
                ...
                for emp in trip.employees:
                    ...
                    results['employees'][emp]['pickup_time'] = self._fmt_time(
                        trip.pickup_times[emp])
                    results['employees'][emp]['dropoff_time'] = self._fmt_time(
                        trip.arrival_at_office)
    ...
    def print_results(self, results: Dict):
        ...
        print(
            f"   All Assigned:       {'✅ YES' if s['all_assigned'] else '❌ NO'}")
        ...
        for sched in results['vehicle_schedules']:
            ...
            print(f"\n╔{'═'*88}╗")
            print(
                f"║ VEHICLE {v['id']} ({v['category']}) - {len(sched['trips'])} trip(s)".ljust(89) + "║")
            print(f"║   Start: {v['start_location']} at {v['available_from']} | Capacity: {v['capacity']}".ljust(
                89) + "║")
            print(f"╠{'═'*88}╣")
            ...
            for trip in sched['trips']:
                ...
                print(
                    f"║   Employees: {', '.join(trip['employees'])}".ljust(89) + "║")
                print(
                    f"║   Sequence: {' → '.join(trip['pickup_sequence'])} → Office".ljust(89) + "║")
                print(
                    f"║   Start: {trip['start_location']} at {trip['start_time']}".ljust(89) + "║")
                print(
                    f"║   Arrive Office: {trip['arrival_at_office']}".ljust(89) + "║")
                print(f"║   Distance: {trip['distance_km']} km | Cost: ₹{trip['cost']:.2f}".ljust(
                    89) + "║")

                t = trip['constraints']['time']
                t_status = '✅' if t['satisfied'] else '❌'
                print(f"║   {t_status} Time: deadline={self._fmt_time(t['deadline'])}, arrival={self._fmt_time(t['arrival_at_office'])}, slack={t['slack']:.1f}min".ljust(
                    89) + "║")

                c = trip['constraints']['capacity_sharing']
                c_status = '✅' if c['satisfied'] else '❌'
                print(f"║   {c_status} Capacity: n={c['n']} <= max_allowed={c['max_allowed']}".ljust(
                    89) + "║")

                vt = trip['constraints']['vehicle_type']
                vt_status = '✅' if vt['satisfied'] else '❌'
                print(f"║   {vt_status} Type: '{vt['vehicle_category']}' ∈ {vt['allowed_intersection']}".ljust(
                    89) + "║")
        ...
        print(f"│ {c4_status} CONSTRAINT 4: ALL EMPLOYEES PICKED EXACTLY ONCE".ljust(
            89) + "│")
        print(
            f"│   {results['summary']['employees_assigned']}/{results['summary']['total_employees']} employees assigned".ljust(89) + "│")
        print(f"└{'─'*88}┘")
# =============================================================================
# MAIN
# =============================================================================
...
def optimize(filepath: str, verbose: bool = True) -> Dict:
    ...
    if verbose:
        ...
        print(
            f"Metadata: α={metadata.get('alpha', 0.7)}, β={metadata.get('beta', 0.3)}")
